...
plot_static_layer <- function(
  ...
  if (aoi_only) {
    ...
    if(params$bins > 0 && is.null(params$breaks)) {
      ...
      params$breaks <- break_pretty2(
        data = vals, n = params$bins + 1, FUN = signif,
        method = params$binning_method %>% {if(is.null(.)) "quantile" else .},
        quantiles = params$quantiles, min_value = params$min_value)
    }
  ...
  baseplot <- if (is.null(baseplot) || identical(baseplot, "vector")) {
    ggplot() +
      ...
      annotation_map_tile(type = "https://a.basemaps.cartocdn.com/light_nolabels/${z}/${x}/${y}.png", zoom = get_zoom_level(static_map_bounds) + zoom_adj, progress = "none")
  ...
  # Roads are drawn by add_roads() in maps-static.R now, this path is dead.
  # if (plot_roads) p <- p +
  #   geom_spatvector(data = roads, aes(linewidth = road_type), color = "grey55") +
  ...
  if (plot_wards) {
    # Light grey fill, no stroke — uses 4-coloring computed in setup.R so no
    # bordering ADM2 polygons share the same shade. Falls back to dashed stroke
    # if ward_fill column wasn't computed.
    if ("ward_fill" %in% names(wards)) {
      p <- p +
        ggnewscale::new_scale_fill() +
        geom_spatvector(data = wards, aes(fill = ward_fill), color = NA, alpha = 0.5) +
        scale_fill_identity(guide = "none")
      # Move ward layer right after the basemap tile so it sits under the data geom.
      n <- length(p$layers)
      tile_idx <- which(vapply(p$layers[seq_len(n - 1)], function(l) {
      ...
      p <- p + geom_spatvector(data = wards, color = "grey25", fill = NA, linetype = "C4", linewidth = 0.15)
    }
    ...
    # Ward centroid labels disabled — looked too cluttered against AOI/data.
    # if ("ward_label" %in% names(wards)) {
    #   ward_pts <- terra::centroids(wards)
    #   ward_lbl_df <- data.frame(
    #     x = terra::geom(ward_pts)[, "x"],
    #     y = terra::geom(ward_pts)[, "y"],
    #     label = as.data.frame(wards)$ward_label
    #   )
    #   p <- p + geom_spatial_text(data = ward_lbl_df, crs = "epsg:4326",
    ...
    #     inherits(l$geom, "GeomMapTile") || inherits(l$stat, "StatMapTile")
    #   }, logical(1)))
    #   insert_after <- if (length(tile_idx) > 0) max(tile_idx) + 1 else 2
    #   if (insert_after < n - 1) {
    #     new_order <- c(seq_len(insert_after), n, seq(insert_after + 1, n - 1))
    #     p$layers <- p$layers[new_order]
    #   }
...
# Helper: overlay city name labels with leader lines, all sitting OUTSIDE the AOI.
# Reads `city_labels` and `aoi` from global env (both loaded in setup.R).
# For each city centroid, computes the nearest point on the AOI buffered by
# `nudge_km` — that point sits just outside the AOI, becomes the label's seed
# position. ggrepel still applies repulsion to keep labels off each other.
add_city_labels <- function(p, label_col = "GC_UCN_MAI_2025", nudge_km = 50) {
  if (!exists("city_labels") || is.null(city_labels)) return(p)

  cl_3857 <- sf::st_transform(city_labels, "EPSG:3857")
  centroids <- suppressWarnings(sf::st_centroid(cl_3857))
  cl_xy <- sf::st_coordinates(centroids)

  # AOI boundary, buffered outward by nudge_km — labels sit on this ring
  aoi_3857 <- sf::st_transform(sf::st_as_sf(aoi), "EPSG:3857")
  aoi_buf_bnd <- sf::st_boundary(sf::st_union(sf::st_buffer(aoi_3857, dist = nudge_km * 1000)))

  # Nearest point on the buffered boundary per centroid → label seed position
  near_lines <- sf::st_nearest_points(centroids, aoi_buf_bnd)
  near_xy <- t(vapply(seq_along(near_lines), function(i) {
    coords <- sf::st_coordinates(near_lines[i])
    as.numeric(coords[nrow(coords), c("X", "Y")])
  }, numeric(2)))

  df <- data.frame(
    X = cl_xy[, "X"],
    Y = cl_xy[, "Y"],
    label = centroids[[label_col]],
    nudge_x_m = near_xy[, 1] - cl_xy[, "X"],
    nudge_y_m = near_xy[, 2] - cl_xy[, "Y"],
  ...
  p + ggrepel::geom_text_repel(
    ...
    inherit.aes = FALSE,
    nudge_x = df$nudge_x_m,
    nudge_y = df$nudge_y_m,
    direction = "x",
    force = 5, force_pull = 0.05, max.overlaps = Inf,
    box.padding = 0.5, point.padding = 0.3,
    segment.color = "grey30", segment.size = 0.3, segment.linetype = "dotted",
...
# layer "major_roads"). Lobito-specific. Dark grey, thin lines so it sits as a
# context layer beneath the city labels but above the basemap.
add_roads <- function(p, underlay = TRUE, color = "grey65") {
  if (!exists("road_network") || is.null(road_network)) return(p)
  roads_sf <- sf::st_as_sf(road_network)
  # Clip roads to AOI so they don't extend past the corridor boundary
  if (exists("aoi") && !is.null(aoi)) {
    aoi_sf <- sf::st_as_sf(aoi)
    if (sf::st_crs(roads_sf) != sf::st_crs(aoi_sf)) {
      roads_sf <- sf::st_transform(roads_sf, sf::st_crs(aoi_sf))
    }
    roads_sf <- suppressWarnings(sf::st_intersection(roads_sf, aoi_sf))
  }
  # Coalesce NA highway → "primary" so untagged features still draw.
  roads_sf$highway <- ifelse(is.na(roads_sf$highway), "primary", roads_sf$highway)
  width_map <- c(
    "motorway" = 0.5, "trunk" = 0.4, "primary" = 0.28,
    "connector" = 0.33,
    "secondary" = 0.22, "tertiary" = 0.17, "unclassified" = 0.11,
    "motorway_link" = 0.19, "trunk_link" = 0.17, "primary_link" = 0.14,
    "secondary_link" = 0.11, "tertiary_link" = 0.08
  )
  ...
  if (underlay) {
    n <- length(p$layers)
    # Insert right after the basemap tile layer so roads sit above tiles but
    # below the data geom. plot_static_layer builds layers as:
    #   1: geom_spatvector(static_map_bounds)  2: annotation_map_tile  3: data geom  ...
...
vectorize_if_coarse <- function(data, threshold = 7000) {
  ...
  if (cell_count < threshold) {
    data <- rast_as_vect(data)
    # Clip polygon edges to AOI so coarse pixels don't overhang the boundary
    if (exists("aoi") && !is.null(aoi)) {
      data <- tryCatch(terra::crop(data, aoi), error = function(e) data)
    }
  }
  return(data)
}
