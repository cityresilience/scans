...
for (td in task_dirs) {
  ...
  if (file.exists(yml)) {
    ...
    for (lyr_raw in task_maps$layers) {
      # Layers may be plain strings OR dict-form per-layer overrides like:
      #   - wsf_harmonized_recent:
      #       aggregate_mode: hexbin
      #       aggregate_fun: q25
      if (is.list(lyr_raw)) {
        lyr <- names(lyr_raw)[1]
        ov  <- lyr_raw[[1]]
        cur_afun  <- ov$aggregate_fun  %||% afun
        cur_amode <- ov$aggregate_mode %||% amode
        cur_asize <- ov$aggregate_size %||% asize
        cur_acov  <- ov$min_coverage   %||% acov
        cur_smooth <- ov$smoothing     %||% smooth_cfg
      } else {
        lyr <- lyr_raw
        cur_afun <- afun; cur_amode <- amode; cur_asize <- asize
        cur_acov <- acov; cur_smooth <- smooth_cfg
      }
      aggregate_fun_lookup[[lyr]] <- cur_afun
      if (!is.null(cur_amode))  aggregate_mode_lookup[[lyr]]     <- cur_amode
      if (!is.null(cur_asize))  aggregate_size_lookup[[lyr]]     <- cur_asize
      if (!is.null(cur_acov))   aggregate_coverage_lookup[[lyr]] <- cur_acov
      if (!is.null(cur_smooth)) smoothing_lookup[[lyr]]          <- cur_smooth
    }
  }
}
...
standard_layers %>%
  map2(names(.), \(fuzzy_string, yaml_key) {
    tryCatch_named(yaml_key, {
      ...
      # Clip vectors to AOI too (e.g. Meta RWI shapefile grid extends past corridor)
      ...
      # If vectorize_if_coarse returned a SpatVector, plot that (edges clipped
      # to AOI). Otherwise keep the raster. Avoids coarse-pixel overhang past AOI.
      plot_data <- if (inherits(raster_data, "SpatVector")) raster_data else data

      if (nrow(data) > 0) {
        plot <- plot_static_layer(
          data = plot_data, yaml_key = yaml_key,
